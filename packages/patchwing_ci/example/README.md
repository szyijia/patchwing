...
`patchwing_ci.yaml` is the workflow `patchwing_ci generate` produces for
...
patchwing_ci generate --repo-root . --dry-run
```

The workflow has two jobs. `setup` activates `patchwing_ci` on the
