...
import 'package:patchwing_ci/src/action_versions.dart';
import 'package:patchwing_ci/src/commands/repo_root_option.dart';
import 'package:patchwing_ci/src/dependency_resolver.dart';
import 'package:patchwing_ci/src/flutter_version_resolver.dart';
import 'package:patchwing_ci/src/package_description.dart';
import 'package:patchwing_ci/src/package_slug.dart';
import 'package:patchwing_ci/src/repository_analyzer.dart';
import 'package:patchwing_ci/src/repository_description.dart';
  ...
  GenerateCommand({LatestMajorResolver? resolveLatestMajor})
    ...
    argParser
      ..addOption(
        ...
        defaultsTo: '.github/workflows/patchwing_ci.yaml',
      )
      ..addOption(
        'style',
...
resolves to `required`. Skip the flag and any package slug is fine.''',
  ...
  Future<int> run() async {
    ...
    // run `patchwing_ci update_actions` to bump them.
...
''');
  ...
  Map<String, String> _buildStaticFiles(
    ...
    if (hasDart) {
      files['.github/workflows/_patchwing_ci_dart.yaml'] =
    ...
    if (hasFlutter) {
      files['.github/workflows/_patchwing_ci_flutter.yaml'] =
  ...
  String _buildStaticMainYaml({
    ...
    final buffer = StringBuffer()
      ..write('''
# Generated by patchwing_ci --style static. Safe to edit.
# Run `patchwing_ci verify` to check for dep graph drift.
name: Patchwing CI
...
''');
    ...
    steps:
      ...
      - run: dart pub global activate patchwing_ci
      - name: Verify CI coverage
        run: patchwing_ci verify
...
''');
    ...
    for (final package in packages) {
      ...
      final reusable = isFlutter
          ? '_patchwing_ci_flutter.yaml'
          : '_patchwing_ci_dart.yaml';
      final slug = slugs[package]!;
...
''';
  ...
  String _buildDynamicYaml(
    ...
    final buffer = StringBuffer()
      ..write('''
# Generated by patchwing_ci. Safe to edit.
# Run `patchwing_ci verify` to check for dep graph drift.
# patchwing_ci-managed: dynamic
name: Patchwing CI
...
''');
    if (hasDart) {
      buffer.write(r'''
          DART=$(patchwing_ci affected_packages --sdk dart $EXTRA_ARGS)
          echo "dart_packages=$DART" >> $GITHUB_OUTPUT
''');
    }
    if (hasFlutter) {
      buffer.write(r'''
          FLUTTER=$(patchwing_ci affected_packages --sdk flutter $EXTRA_ARGS)
