...
```sh
dart pub global activate patchwing_ci
```

## Quick Start

```sh
patchwing_ci generate --dry-run            # review what it would write
patchwing_ci generate --required           # write + add a single aggregator check
```
...
| Command | Purpose |
|---|---|
| `generate` | Write `.github/workflows/patchwing_ci.yaml` (plus reusable workflows in `--style static`) |
...
activate`s `patchwing_ci`, runs `verify` as a sanity check, then runs
...
```sh
patchwing_ci generate --codecov-token-secret CODECOV_TOKEN
```
...
```sh
patchwing_ci verify --ignore e2e,other_package
```
