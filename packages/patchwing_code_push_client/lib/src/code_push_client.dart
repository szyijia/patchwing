...
import 'package:patchwing_code_push_client/src/version.dart';
...
class CodePushClient {
  ...
  CodePushClient({
    ...
    Map<String, String>? customHeaders,
       ...
       // Always use a brand-new http.Client for raw requests (e.g. MinIO
       // presigned URL uploads) to avoid inherited headers such as
       // Authorization from an authenticated client.
  ...
  /// Raw HTTP client without extra headers (e.g. x-version, Authorization).
  /// Used for MinIO presigned URL uploads where extra headers would break
  ...
  /// Parses the JSON response body, automatically unwrapping the standard
  /// backend envelope `{code, data, msg}` if present.
  /// Returns the `data` field when the wrapper is detected, otherwise returns
  /// the raw decoded JSON.
  /// Throws [CodePushException] if the backend returns a non-zero code.
  Map<String, dynamic> _decodeResponseBody(String body) {
    final decoded = json.decode(body) as Map<String, dynamic>;
    if (decoded.containsKey('data') && decoded.containsKey('code')) {
      final code = decoded['code'] as int?;
      if (code != null && code != 0) {
        final msg = decoded['msg'] as String? ?? unknownErrorMessage;
        throw CodePushException(message: msg);
      }
  ...
  Future<void> createPatchArtifact({
    ...
    final decoded = CreatePatchArtifactResponse.fromJson(
      _decodeResponseBody(body),
    );

    // Upload artifact to MinIO using PUT (matching MinIO presigned PUT URL).
    // Use _rawHttpClient to avoid extra headers (x-version, Authorization)
    // that would break MinIO presigned URL signature validation.
    // Do NOT set Content-Type header because the presigned URL only signed
    // the 'host' header (X-Amz-SignedHeaders=host); adding any extra signed
    // header will cause a 400 Bad Request from MinIO.
    final artifactFile = File(artifactPath);
    final fileBytes = await artifactFile.readAsBytes();
    final uploadRequest = http.Request('PUT', Uri.parse(decoded.url))
      ..bodyBytes = fileBytes;

    final uploadResponse = await _rawHttpClient.send(uploadRequest);

    if (!uploadResponse.isSuccess) {
      final errBody = await uploadResponse.stream.bytesToString();
      throw CodePushException(
        message:
            '''Failed to upload artifact (${uploadResponse.reasonPhrase} '${uploadResponse.statusCode}): $errBody''',
      );
    }
  }
  ...
  Future<List<Channel>> getChannels({required String appId}) async {
    ...
    final body = _decodeResponseBody(response.body);
    final channels = body['channels'] as List? ?? body.values.toList();
    return channels
  ...
  /// Get all patch artifacts for a specific [patchId].
  Future<List<PatchArtifact>> getPatchArtifacts({
    required String appId,
    required int patchId,
  }) async {
    final response = await _httpClient.get(
    ...
    final decoded = GetPatchArtifactsResponse.fromJson(
      _decodeResponseBody(response.body),
    );
  ...
  /// Returns the auth response with token, api_key, email, and name.
  Future<Map<String, dynamic>> loginPassword({
    required String email,
    required String password,
  }) async {
    final response = await _httpClient.post(
      Uri.parse('$hostedUri/api/v1/auth/login'),
