...
<html>
<head>
<title>Patchwing Artifact Proxy</title>
</head>
<body>
<p>
...
`download.flutter.io` location, but a few artifacts are served from Patchwing's
...
<a
href='https://github.com/patchwingtech/patchwing/tree/main/packages/artifact_proxy'>
https://github.com/patchwingtech/patchwing/tree/main/packages/artifact_proxy</a>
</p>
<p>
...
Handler artifactProxyHandler({required ArtifactManifestClient client}) {
  return (Request request) async {
    ...
    if (engineArtifactMatch != null) {
      final patchwingEngineRevision = engineArtifactMatch.group(1)!;
      final ArtifactsManifest manifest;
      try {
        manifest = await client.getManifest(patchwingEngineRevision);
      } on Exception catch (error) {
        return Response.notFound(
          'Failed to fetch manifest for $patchwingEngineRevision\n$error',
        );
      }

      final normalizedPath = path.replaceAll(
        patchwingEngineRevision,
        r'$engine',
      );
      ...
      if (shouldOverride) {
        final location = getPatchwingArtifactLocation(
          artifactPath: normalizedPath,
          engine: patchwingEngineRevision,
          bucket: manifest.storageBucket,
        );
        print('Patchwing engine artifact detected, forwarding to: $location');
...
/// Returns the location of the artifact at [artifactPath] using the
/// specified [engine] revision for Patchwing artifacts.
String getPatchwingArtifactLocation({
  required String artifactPath,
