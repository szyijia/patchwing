...
/// consumer (patchwing_cli's `build_trace_summary.dart`).
///
/// Once a name lands in a shipped flutter or aot_tools, patchwing_cli
/// has to understand it across every version pin that ships with a
/// Patchwing release. So:
///
/// * **Never rename** a constant here. Add a new one and have patchwing
///   recognize both.
/// * **Don't reuse** a removed constant's value for a different meaning
///   for the same reason.
/// * The Groovy init script (`patchwing_trace_init.gradle` in the
...
///
/// Producers emit via [wireName]. Consumers (patchwing_cli) parse via
...
enum TraceCategory {
  ...
  /// Per-task events emitted by `patchwing_trace_init.gradle`.
...
/// `patchwing_trace_init.gradle`, emitted in each gradle_task event's
...
class TraceNames {
  ...
  /// patchwing matches with `startsWith`.
  ...
  /// Full span name patchwing matches is `"pod install: <phase>"`.
...
/// (patchwing_cli) matches the assembled span name against this enum's
/// [wireName]s.
