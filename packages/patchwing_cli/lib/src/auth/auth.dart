...
import 'package:patchwing_cli/src/auth/ci_token.dart';
import 'package:patchwing_cli/src/auth/endpoints/endpoints.dart';
import 'package:patchwing_cli/src/auth/patchwing_oauth.dart' as patchwing_oauth;
...
/// The environment variable that holds the Patchwing CI token.
const patchwingTokenEnvVar = 'PATCHWING_TOKEN';
...
/// Callback for obtaining Patchwing access credentials via loopback login.
...
class AuthenticatedClient extends http.BaseClient {
  ...
  Future<oauth2.AccessCredentials> _refreshForProvider(
    ...
    try {
      // Patchwing uses its own refresh flow; Google and Microsoft use the
      // standard OAuth refresh. This branching can be removed once the
      // Google/Microsoft providers are fully removed from the CLI.
      if (authProvider == AuthProvider.patchwing) {
        return await patchwing_oauth.refreshPatchwingCredentials(
          credentials,
          _baseClient,
      ...
      logger
        ...
        ..info(
          '''Try logging out with ${lightBlue.wrap('patchwing logout')} and logging in again.''',
        )
        ..detail(e.toString())
        ..detail(s.toString());
...
class Auth {
  ...
  Auth({
    ...
    CodePushClientBuilder? buildCodePushClient,
       ...
       _obtainCredentialsViaLoopbackLogin =
           ...
           patchwing_oauth.obtainCredentialsViaLoopbackLogin,
  ...
  /// Logs in the user via the Patchwing loopback OAuth flow.
  Future<void> login({required void Function(String) prompt}) async {
    ...
    try {
      ...
      final codePushClient = _buildCodePushClient(
        ...
        hostedUri: patchwingEnv.hostedUri,
      );
  ...
  ///
  /// Stores the returned API key directly (no refresh flow needed).
  Future<void> loginWithPassword({
    ...
    final result = await codePushClient.loginPassword(
      email: email,
      password: password,
    );

    final apiKey = result['api_key'] as String?;
    final emailResult = result['email'] as String?;
    final nameResult = result['name'] as String?;

    if (apiKey == null || emailResult == null) {
      throw Exception('Login failed: server did not return api_key or email');
    }

    _apiKey = apiKey;
    _email = emailResult;

    // 将 API key 写入 credentials.json，方便后续读取
  ...
  /// Logs out the user.
  ///
  /// If a Patchwing refresh token is available, revokes the server-side
  ...
  void _loadCredentials() {
    final envToken = platform.environment[patchwingTokenEnvVar];
    if (envToken != null) {
      final trimmed = envToken.trim();
      logger.detail('[env] $patchwingTokenEnvVar detected');

      // New API key format — pass through directly, no refresh needed.
      // Support both 'sb_api_' (legacy) and 'pw_' (Patchwing custom) prefixes.
      if (trimmed.startsWith('sb_api_') || trimmed.startsWith('pw_')) {
        _apiKey = trimmed;
        logger.detail('[env] $patchwingTokenEnvVar parsed as API key');
        return;
      }
      ...
      try {
        ...
        logger
          ..err(
            'Failed to parse $patchwingTokenEnvVar. Expected an API key '
      ...
      logger.detail('[env] $patchwingTokenEnvVar parsed as legacy CiToken');
      return;
    }
    ...
    if (credentialsFile.existsSync()) {
      try {
        ...
        // 支持 password-login 存储的 api_key 格式
...
extension OauthAuthProvider on Jwt {
  ...
  AuthProvider get authProvider {
    if (payload.iss == patchwingEnv.jwtIssuer) {
      return AuthProvider.patchwing;
...
extension OauthValues on AuthProvider {
  ...
  oauth2.AuthEndpoints get authEndpoints => switch (this) {
    ...
    (AuthProvider.patchwing) => throw UnsupportedError(
    ...
    switch (this) {
      case AuthProvider.google:
        return oauth2.ClientId(
          /// Patchwing CLI's OAuth 2.0 identifier for GCP,
          '''523302233293-eia5antm0tgvek240t46orctktiabrek.apps.googleusercontent.com''',

          /// Patchwing CLI's OAuth 2.0 secret for GCP.
          ///
      ...
      case AuthProvider.microsoft:
        return oauth2.ClientId(
          /// Patchwing CLI's OAuth 2.0 identifier for Azure/Entra.
          '4fc38981-4ec4-4bd9-a755-e6ad9a413054',
        );
      case AuthProvider.patchwing:
  ...
  List<String> get scopes => switch (this) {
    ...
    // Patchwing auth doesn't use scopes.
    (AuthProvider.patchwing) => [],
  };
}
