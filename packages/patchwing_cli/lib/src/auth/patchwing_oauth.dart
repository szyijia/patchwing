...
/// Exception thrown when the Patchwing auth flow fails.
class PatchwingAuthException implements Exception {
  /// Creates a [PatchwingAuthException] with the given [message].
  const PatchwingAuthException(this.message);
...
/// Implements the full loopback login flow for Patchwing auth.
///
...
Future<HttpRequest> _waitForCallback(
  ...
  try {
    return await completer.future.timeout(
      timeout,
      onTimeout: () {
        throw const PatchwingAuthException(
...
/// callback [request].
///
/// Throws [PatchwingAuthException] if the callback contains an error or is
...
/// Refreshes Patchwing tokens using the refresh token.
///
...
Future<oauth2.AccessCredentials> refreshPatchwingCredentials(
  ...
  if (refreshToken == null) {
    throw const PatchwingAuthException('No refresh token available.');
  }
...
/// issuer matches the expected JWT issuer from [PatchwingEnv].
///
/// Expected JSON shape:
/// ```json
/// {
...
oauth2.AccessCredentials _parseTokenResponse(String responseBody) {
  ...
  try {
    ...
    throw PatchwingAuthException('Invalid access token: ${e.message}');
  }

  // Validate the issuer matches the expected auth service.
  final expectedIssuer = patchwingEnv.jwtIssuer;
