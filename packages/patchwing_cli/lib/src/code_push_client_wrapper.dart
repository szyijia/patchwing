...
import 'package:patchwing_cli/src/patchwing_web_console.dart';
...
class CodePushClientWrapper {
  ...
  Future<App> createApp({required int organizationId, String? appName}) async {
    ...
    if (appName == null) {
      final defaultAppName = patchwingEnv.getPubspecYaml()?.name;
...
This app may not exist or you may not have permission to view it.''');
  ...
  void ensureReleaseIsNotActive({
    ...
    if (release.platformStatuses[platform] == ReleaseStatus.active) {
      final uri = PatchwingWebConsole.appReleaseUri(release.appId, release.id);
      logger.err(
        '''
...
Please create a release using "patchwing release" and try again.
''');
      throw ProcessExit(ExitCode.software.code);
    }
...
Please run `patchwing cache clean` and try again. If the issue persists, please
file a bug report at https://github.com/patchwingtech/patchwing/issues/new.
...
Looked in:
  ...
  - build/app/intermediates/stripped_native_libs/{flavor}Release/out/lib''',
    ...
    // (https://github.com/patchwingtech/patchwing/issues/3388).
...
aar artifact already exists, continuing...''');
  ...
  /// Registers and uploads macOS release artifacts to the Patchwing server.
  ...
  /// Publishes a patch to the Patchwing server. This consists of creating a
  ...
  Never _handleErrorAndExit(
    ...
    if (error is CodePushUpgradeRequiredException) {
      ...
      logger
        ...
        ..info(
          '''Run ${lightCyan.wrap('patchwing upgrade')} to get the latest version.''',
        );
