...
import 'package:patchwing_cli/src/artifact_builder/build_environment.dart';
import 'package:patchwing_cli/src/artifact_builder/build_trace_session.dart';
import 'package:patchwing_cli/src/artifact_builder/build_trace_summary.dart';
...
import 'package:patchwing_cli/src/os/operating_system_interface.dart';
import 'package:patchwing_cli/src/platform.dart' as scoped_platform;
...
/// Used to wrap code that invokes `flutter build` with Patchwing's fork of
/// Flutter.
typedef PatchwingBuildCommand = Future<void> Function();
...
Map<String, String>? buildEnvironment({
  ...
  if (base64PublicKey != null) {
    env['PATCHWING_PUBLIC_KEY'] = base64PublicKey;
  }
  if (ddMaxBytes != null) {
    env['PATCHWING_DD_MAX_BYTES'] = ddMaxBytes.toString();
  }
...
${lightCyan.wrap('patchwing release <platform> --flutter-version=3.29.0')}

• If `flutter build` completes successfully and `patchwing release`
fails when using the same flutter version, please file an issue:
${link(uri: Uri.parse('https://github.com/patchwingtech/patchwing/issues/new'))}
''';

  /// Cache of `flutter build <command>` help output checks for
  /// `--patchwing-trace` support. Populated lazily by
  /// [_supportsTraceFlag].
  final _traceSupport = <String, bool>{};

  /// Returns whether `flutter build <command>` accepts `--patchwing-trace`.
  ///
  ...
  Future<bool> _supportsTraceFlag(String command) async {
    ...
    try {
      ...
      final supported = result.stdout.toString().contains('--patchwing-trace');
  ...
  /// Returns the `--patchwing-trace` argument if the current
  ...
  Future<List<String>> _traceArgs(String command) async {
    ...
    return ['--patchwing-trace=${traceFile.path}'];
  }
  ...
  Future<File> buildAppBundle({
    ...
    await _runPatchwingBuildCommand(() async {
      const executable = 'flutter';
      final targetPlatformArgs = targetPlatforms?.targetPlatformArg;
      // 注意：--local-engine / --local-engine-src-path / --local-engine-host
      // 由 PatchwingProcess._resolveArguments 根据 engineConfig 统一注入，
      // 这里不要再手动拼装，否则会出现两份冲突的 --local-engine-src-path，
      // 导致 flutter 使用错误（指向 patchwing 自己 clone 的 SDK 目录而非
      // 真正的引擎产物目录）。
      final arguments = [
        'build',
        'appbundle',
...
Reason: Exited with code $exitCode.''',
    ...
    try {
      return patchwingAndroidArtifacts.findAab(
        project: projectRoot,
        flavor: flavor,
      );
  ...
  Future<File> buildApk({
    ...
    await _runPatchwingBuildCommand(() async {
      ...
      final arguments = [
        ...
        // See https://github.com/patchwingtech/patchwing/issues/1141.
...
Reason: Exited with code $exitCode.''',
    ...
    final projectRoot = patchwingEnv.getPatchwingProjectRoot()!;
    try {
      return patchwingAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
    } on MultipleArtifactsFoundException catch (error) {
  ...
  Future<void> buildAar({
    ...
    return _runPatchwingBuildCommand(() async {
      const executable = 'flutter';
      final targetPlatformArgs = targetPlatforms?.targetPlatformArg;
      final arguments = [
        'build',
...
Reason: Exited with code $exitCode.''',
  ...
  /// --release` with Patchwing's fork of Flutter.
...
Reason: Exited with code $exitCode.''',
    ...
    if (appDillPath == null) {
      throw ArtifactBuildException(
        ...
        fixRecommendation:
            '''Please file a bug at https://github.com/patchwingtech/patchwing/issues/new with the logs for this command.''',
      );
    }
...
Reason: Exited with code $exitCode.''',
  ...
  /// the `patchwingEnv` override is active so the version gate checks the
  /// correct revision.
  ///
  /// No-op on older Flutter pins that don't support `--patchwing-trace`
  ...
  Future<void> prepareBuildTrace({required String platform}) async {
    ...
    final flutterVersion = await patchwingFlutter.resolveFlutterVersion(
      revision,
    );
    ...
    final traceFile = File(
      p.join(
        patchwingEnv.buildDirectory.path,
        'patchwing',
        'debug',
  ...
  /// Emits a flow-start event (`ph: "s"`) on the patchwing_cli tracer
  /// tied to the spawned flutter process's real pid. When flutter builds
  /// with `--patchwing-trace`, it records a flow-end with its own pid as
  ...
  void _emitFlutterSpawnFlow(Process flutter) {
    patchwingTracer.addSpawnFlowStart(id: flutter.pid, at: clock.now());
  }
  ...
  /// Patchwing itself spent around the Flutter build, subtracting Flutter's
  ...
  void writeBuildTraceSummary() {
    ...
    // derive Patchwing's overhead. Second pass (below) then bakes overhead
    ...
    final patchwingOverhead = totalElapsed - flutterBuild;
    ...
    final environment = BuildEnvironment.detect(
      ...
      projectRoot: patchwingEnv.getPatchwingProjectRoot(),
    );
    ...
    final summary = BuildTraceSummary.fromEvents(
      events,
      platform: buildPlatform,
      patchwingOverhead: patchwingOverhead.isNegative
          ? Duration.zero
          : patchwingOverhead,
  ...
  /// Patchwing's fork of Flutter) with a try/finally that runs
  /// `flutter pub get` with the system installation of Flutter to reset
  /// `.dart_tool/package_config.json` to the system Flutter.
  Future<void> _runPatchwingBuildCommand(PatchwingBuildCommand command) async {
    try {
      await command();
    } finally {
  ...
  /// run by patchwing update the package_config.json file to point to
  /// patchwing's version of Flutter, which confuses VS Code. See
  /// https://github.com/patchwingtech/patchwing/issues/1101 for more info.
...
''');
  ...
  Future<File> buildElfAotSnapshot({
    ...
    required PatchwingArtifact genSnapshotArtifact,
    ...
    // Record a span on the patchwing_cli row so gen_snapshot time shows up
    ...
    final exitCode = await patchwingTracer.span<int>(
      name: 'gen_snapshot',
      category: 'subprocess',
      body: () => process.stream(
        patchwingArtifacts.getArtifactPath(artifact: genSnapshotArtifact),
        arguments,
        // Never run in shell because we always have a fully resolved
        // executable path.
        runInShell: false,
