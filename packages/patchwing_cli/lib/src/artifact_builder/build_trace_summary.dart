...
// `patchwing_build_trace`'s `TraceSchema`; if you add a new bucket
...
import 'package:patchwing_cli/src/artifact_builder/duration_distribution.dart';

/// Summary of a Chrome Trace Event Format build trace.
///
/// Aggregate [Duration] timings and small integer counters, suitable
/// for uploading to Patchwing's servers as part of release telemetry.
class BuildTraceSummary {
  ...
  BuildTraceSummary({
    ...
    required this.patchwingOverhead,
  ...
  factory BuildTraceSummary.fromEvents(
    ...
    return _buildSummary(
      acc,
      platform: platform,
      patchwingOverhead: patchwingOverhead,
  ...
  static BuildTraceSummary _buildSummary(
    ...
    return BuildTraceSummary(
      platform: platform,
      total: flutterBuild + (patchwingOverhead ?? Duration.zero),
  ...
  /// Patchwing overhead computed from it) should parse once and pass the
  ...
  /// Total command wall-clock (Flutter build + Patchwing overhead).
  ...
  /// Wall-clock time the Patchwing CLI spent around Flutter. Null when
  /// the caller couldn't compute it.
  final Duration? patchwingOverhead;

  /// Network I/O time and request counts, summed across Patchwing-side
  ...
  /// Patchwing CLI's wall-clock time around Flutter with network I/O
  /// subtracted — i.e. what Patchwing spent doing local work (file I/O,
  /// hashing, archive assembly, aot_tools link/gen_snapshot bookkeeping
  /// outside their own spans). Null when [patchwingOverhead] is null.
  ///
  ...
  /// overhead).
  Duration? get patchwingLocal {
    final overhead = patchwingOverhead;
  ...
  Map<String, Object?> toJson() => <String, Object?>{
    ...
    'patchwingOverheadMs': patchwingOverhead?.inMilliseconds,
    'patchwingLocalMs': patchwingLocal?.inMilliseconds,
...
/// Network I/O totals. Combined across Patchwing-side (auth, artifact
...
/// `patchwing_trace_init.gradle` TaskExecutionListener.
class GradleStats {
