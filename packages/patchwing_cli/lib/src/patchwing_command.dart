...
import 'package:patchwing_cli/src/interactive_mode.dart' as interactive_mode;
...
/// {@template patchwing_command}
/// A command in the Patchwing CLI.
/// {@endtemplate}
abstract class PatchwingCommand extends Command<int> {
  // We don't currently have a test involving both a CommandRunner
  // and a Command, so we can't test this getter.
  // coverage:ignore-start
  @override
  PatchwingCliCommandRunner? get runner =>
      testRunner ?? super.runner as PatchwingCliCommandRunner?;
  // coverage:ignore-end
  ...
  @visibleForTesting
  PatchwingCliCommandRunner? testRunner;
  ...
  // "interactive mode" matrix. Exercising it through a PatchwingCommand
  ...
  ///   `'patchwing releases list --app-id <id> --json'`
  ...
  Future<({String appId, int? errorCode})> resolveAppId() async {
    ...
    try {
      await patchwingValidator.validatePreconditions(
        ...
        checkPatchwingInitialized: explicitAppId == null,
      );
    ...
    final appId =
        explicitAppId ??
        patchwingEnv.getPatchwingYaml()!.getAppId(flavor: flavor);
...
/// {@template patchwing_proxy_command}
/// A command in the Patchwing CLI that proxies to an underlying process.
/// {@endtemplate}
abstract class PatchwingProxyCommand extends PatchwingCommand {
  @override
