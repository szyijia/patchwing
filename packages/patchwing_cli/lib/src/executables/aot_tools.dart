...
Verify that `patchwing patch` was invoked with the exact same --dart-define
(and --dart-define-from-file) flags as `patchwing release`, and that the
...
/// Wrapper around the patchwing `aot-tools` executable.
class AotTools {
  ...
  static bool usesLinker(String flutterRevision) {
    ...
    // This will be wrong if we ever need new patchwing to work with old local
    // engine build.
  ...
  Future<PatchwingProcessResult> _exec(
    List<String> command, {
    ...
    // Trace Event Format file as Flutter and patchwing_cli. Prepended
    ...
    final artifactPath = patchwingArtifacts.getArtifactPath(
      artifact: PatchwingArtifact.aotTools,
    );

    final PatchwingProcessResult result;

    // Similar to [PatchwingProcess.run] but uses [PatchwingProcess.start]
    // instead and captures the live stdout and stderr of the process.
    Future<PatchwingProcessResult> execute(
      String exe,
      List<String> args, {
      String? workingDirectory,
    }) async {
    ...
    // Fallback behavior for older versions of patchwing where aot-tools was
    ...
    if (extension != '.dill' && extension != '.dart') {
      ...
      result = await execute(patchwingEnv.dartBinaryFile.path, [
        'run',
        artifactPath,
        ...tracedCommand,
      ], workingDirectory: workingDirectory);
