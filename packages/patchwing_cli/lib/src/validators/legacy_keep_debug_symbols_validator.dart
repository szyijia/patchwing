...
/// `keepDebugSymbols` exclusion to keep `libapp.so` byte-stable for Patchwing
...
class LegacyKeepDebugSymbolsValidator extends Validator {
  ...
  Future<List<ValidationIssue>> validate() async {
    ...
    for (final gradleFile in _gradleFiles) {
      ...
      final relativePath = p.relative(
        gradleFile.path,
        from: patchwingEnv.getFlutterProjectRoot()!.path,
      );
      issues.add(
        ValidationIssue(
  ...
  Directory? get _androidAppDirectory {
    final root = patchwingEnv.getFlutterProjectRoot();
