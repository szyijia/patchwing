...
class AndroidPatcher extends Patcher {
  ...
  /// https://github.com/patchwingtech/updater/issues/211 was fixed in 3.24.2
...
See more info about the issue ${link(uri: Uri.parse('https://github.com/patchwingtech/updater/issues/211'), message: 'on Github')}
''';
  ...
  Future<File> buildPatchArtifact({String? releaseVersion}) async {
    final flutterVersion = await patchwingFlutter.getVersion();
    // Android versions prior to 3.24.2 have a bug that can cause patches to
    // be erroneously uninstalled.
    // https://github.com/patchwingtech/updater/issues/211 was fixed in 3.24.2
...
Looked in:
  ...
  Future<Map<Arch, PatchArtifactBundle>> createPatchArtifacts({
    ...
    // Direct users to https://github.com/patchwingtech/patchwing/issues/2532
...
Please refer to ${link(uri: Uri.parse('https://github.com/patchwingtech/patchwing/issues/2532'))} for potential workarounds.''',
        );
      }),
    );
  ...
  Future<String> extractReleaseVersionFromArtifact(File artifact) async {
    return patchwingAndroidArtifacts.extractReleaseVersionFromAppBundle(
      artifact.path,
    );
  }
}
