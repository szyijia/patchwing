...
import 'package:patchwing_cli/src/formatters/formatters.dart';
...
/// A command that creates a patchwing patch for the provided target platforms.
/// `patchwing patch --platforms=android,ios`
/// {@endtemplate}
class PatchCommand extends PatchwingCommand {
  /// {@macro patch_command}
...
NOTE: this is ${styleBold.wrap('not')} recommended. Asset changes cannot be included in a patch can cause your app to behave unexpectedly.''';
  ...
  Future<void> createPatch(Patcher patcher) async {
    ...
    } else if (patchwingEnv.canAcceptUserInput) {
...
+-------------------------------------------------------------------------------+
''');
      lastBuiltFlutterRevision = patchwingEnv.flutterRevision;
    ...
    try {
      await patchwingFlutter.installRevision(revision: release.flutterRevision);
    } on Exception {
    ...
    if (obfuscationMapFile != null) {
      ...
      final shouldPreStripInGenSnapshot = await patchwingFlutter
...
''');
        ...
        final baseMetadata = CreatePatchMetadata(
          ...
          environment: BuildEnvironmentMetadata(
            ...
            usesPatchwingCodePushPackage:
                patchwingEnv.usesPatchwingCodePushPackage,
          ),
          // Attach the build-trace summary if the build produced one.
          // Null for older Flutter pins without the --patchwing-trace
  ...
  void assertReleaseContainsPlatform({
    ...
    if (!contains) {
      ...
      logger.err(
        '''No release exists for $platformName in release version ${release.version}. Please run patchwing release $platformName to create one.''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }
