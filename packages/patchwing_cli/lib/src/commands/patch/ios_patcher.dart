...
import 'package:patchwing_cli/src/commands/patch/apple_patcher_mixin.dart';
...
class IosPatcher extends Patcher
  ...
  String get _aotOutputPath =>
      p.join(patchwingEnv.buildDirectory.path, 'out.aot');

  String get _vmcodeOutputPath =>
      p.join(patchwingEnv.buildDirectory.path, 'out.vmcode');

  String get _appDillCopyPath =>
      p.join(patchwingEnv.buildDirectory.path, 'app.dill');
  ...
  @override
  String? get localPodfileLockHash => patchwingEnv.iosPodfileLockHash;
  ...
  Future<File> buildPatchArtifact({String? releaseVersion}) async {
    ...
    final (flutterVersionAndRevision, flutterVersion) = await (
      patchwingFlutter.getVersionAndRevision(),
      patchwingFlutter.getVersion(),
    ).wait;
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}''');
    ...
    await artifactBuilder.buildElfAotSnapshot(
      ...
      genSnapshotArtifact: PatchwingArtifact.genSnapshotIos,
      additionalArgs: [
  ...
  Future<Map<Arch, PatchArtifactBundle>> createPatchArtifacts({
    ...
    final useLinker = AotTools.usesLinker(patchwingEnv.flutterRevision);
    if (useLinker) {
      apple.copySupplementFilesToSnapshotDirs(
        ...
        patchSupplementDir: patchwingEnv.iosSupplementDirectory,
        patchSnapshotDir: patchwingEnv.buildDirectory,
      );
      ...
      final result = await apple.runLinker(
        ...
        ddMaxBytes: int.tryParse(
          platform.environment['PATCHWING_PATCH_DD_MAX_BYTES'] ?? '',
        ),
      );
    ...
    if (useLinker && await aotTools.isGeneratePatchDiffBaseSupported()) {
      ...
      final analyzeSnapshotPath = patchwingArtifacts.getArtifactPath(
    ...
    return {
      Arch.arm64: PatchArtifactBundle(
        ...
        podfileLockHash: patchwingEnv.iosPodfileLockHash,
      ),
    };
  }
