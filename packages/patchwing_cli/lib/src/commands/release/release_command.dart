...
import 'package:patchwing_cli/src/extensions/string.dart';
...
/// {@endtemplate}
class ReleaseCommand extends PatchwingCommand {
...
Defaults to "latest" which builds using the latest stable Flutter version.''',
      ...
      // Added for https://github.com/patchwingtech/patchwing/issues/3223.
...
of the iOS app that is using this module. (aar and ios-framework only)''',
  ...
  /// The patchwing app ID for the current project.
  String get appId => patchwingEnv.getPatchwingYaml()!.getAppId(flavor: flavor);
  ...
  /// The workflow to create a new release for a Patchwing app.
  ///
  ...
  Future<void> createRelease(Releaser releaser) async {
    ...
    try {
      await patchwingValidator.validateFlavors(
        flavorArg: flavor,
    ...
    try {
      await patchwingFlutter.installRevision(revision: targetFlutterRevision);
    } on Exception {
    ...
    final releaseFlutterPatchwingEnv = patchwingEnv.copyWith(
    ...
    return await runScoped(
      () async {
        ...
        final flutterVersionString = await patchwingFlutter
...
✅ Published Release ${release.version}!''')
      ...
      values: {patchwingEnvRef.overrideWith(() => releaseFlutterPatchwingEnv)},
    );
  }
  ...
  Future<void> assertArgsAreValid(Releaser releaser) async {
    ...
    if (patchwingYaml?.patchVerification != null && !hasPublicKey) {
      logger.warn(
    ...
    final version = await patchwingFlutter.resolveFlutterVersion(
      flutterVersionArg,
    );
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}''');
  ...
  /// [patchwingEnv]. Will exit with [ExitCode.software] if the version
  /// specified by the user is not found/supported.
  Future<String> resolveTargetFlutterRevision() async {
    if (flutterVersionArg == 'latest') return patchwingEnv.flutterRevision;

    // Fetch the latest remote refs so that release branch pointers
    // (e.g. flutter_release/3.38.5) are up to date.
    await patchwingFlutter.fetchRemoteRefs();

    final String? revision;
    try {
      revision = await patchwingFlutter.resolveFlutterRevision(
        flutterVersionArg,
      );
...
$error''');
    ...
    if (revision == null) {
      final openIssueLink = link(
        uri: Uri.parse(
          'https://github.com/patchwingtech/patchwing/issues/new?assignees=&labels=feature&projects=&template=feature_request.md&title=feat%3A+',
        ),
        message: 'open an issue',
      );
      logger.err('''
Version $flutterVersionArg not found. Please $openIssueLink to request a new version.
Use `patchwing flutter versions list` to list available versions.
''');
      throw ProcessExit(ExitCode.software.code);
    }
  ...
  Future<void> ensureVersionIsReleasable({
    ...
    if (existingRelease != null) {
      ...
      if (existingRelease.flutterRevision != flutterRevision) {
        final flutterVersion = await patchwingFlutter.getVersionForRevision(
          flutterRevision: flutterRevision,
        );

        final formattedCurrentReleaseVersion = patchwingFlutter.formatVersion(
          revision: flutterRevision,
          version: flutterVersion,
        );

        final formattedExistingReleaseVersion = patchwingFlutter.formatVersion(
...
To resolve this issue, you can:
  ...
  * Bump the release version and re-run the release command with the desired Flutter version.''',
  ...
  Future<void> printReleaseSummary({
    ...
    final flutterVersionString = await patchwingFlutter.getVersionAndRevision();
...
''');
    ...
    if (confirm && patchwingEnv.canAcceptUserInput) {
  ...
  Future<Release> getOrCreateRelease({
    ...
    return await codePushClientWrapper.maybeGetRelease(
        ...
        await codePushClientWrapper.createRelease(
          appId: appId,
          version: version,
          flutterRevision: patchwingEnv.flutterRevision,
          platform: releasePlatform,
        );
  }
  ...
  Future<void> finalizeRelease({
    ...
    final baseMetadata = UpdateReleaseMetadata(
      ...
      environment: BuildEnvironmentMetadata(
        ...
        patchwingVersion: packageVersion,
        patchwingYaml: patchwingEnv.getPatchwingYaml()!,
        usesPatchwingCodePushPackage: patchwingEnv.usesPatchwingCodePushPackage,
      ),
      // Attach the build-trace summary if the build produced one.
      // Null for older Flutter pins without the --patchwing-trace flag
