...
Please comment and upvote ${link(uri: Uri.parse('https://github.com/patchwingtech/patchwing/issues/1141'))} if you would like patchwing to support this.''',
        );
  ...
  Future<String> getReleaseVersion({
    ...
    try {
      releaseVersion = await patchwingAndroidArtifacts
  ...
  Future<void> uploadReleaseArtifacts({
    ...
    final aabFile = patchwingAndroidArtifacts.findAab(
      project: projectRoot,
      flavor: flavor,
    );
  ...
  String get postReleaseInstructions {
    ...
    if (generateApk) {
      final apkFile = patchwingAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
      apkText = generateApk
