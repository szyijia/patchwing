...
/// {@endtemplate}
class PromoteCommand extends PatchwingCommand {
  ...
  Future<int> run() async {
    ...
    if (isJsonMode) {
      emitJsonError(
        ...
        message:
            'patchwing patches promote is deprecated and does not support '
            '--json output.',
        hint: 'Use `patchwing patches set-track --track=stable` instead.',
      );
      return ExitCode.usage.code;
    }

    logger.warn(
      '''This command is deprecated and will be removed in a future release. Use `patchwing patches set-track --track=stable` instead.''',
    );
    ...
    final appId = patchwingEnv.getPatchwingYaml()!.getAppId(flavor: flavor);
...
This is a bug and should never happen. Please file an issue at https://github.com/patchwingtech/patchwing/issues/new?assignees=&labels=bug&projects=&template=bug_report.md&title=fix%3A+''',
      );
