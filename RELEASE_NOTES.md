...
- 🐛 macOS: fix `patchwing release --platforms macos` failing with "could not find an analyze_snapshot binary whose --sdk_version matches gen_snapshot". DD activation is skipped on macOS for now (patches will compute DD on the fly when applied, matching pre-1.6.99 behavior). iOS / Android / Linux / Windows are unaffected.

## 1.6.101 (May 19, 2026)

- 🐦 Flutter 3.44.0 / Dart 3.12.0 support
  - 💥 Android: AGP now strips `libapp.so` and emits `libapp.so.sym` (for Play Console crash symbolication). If your `android/app/build.gradle(.kts)` contains a legacy `packaging.jniLibs.keepDebugSymbols.add("**/libapp.so")` line, remove it — `patchwing doctor` flags it for you.
...
- 💥 BREAKING: `patchwing login:ci` now hard-errors. Use API keys instead.
...
- ✨ New `patchwing account apps` subcommand
...
- ✨ New `account` command: `patchwing account whoami` and `patchwing account orgs`
...
- 🔧 Updater: graceful handling of concurrent updates and PATCHWING_NO_UPDATE
...
- 🐛 Fix: only pass --patchwing-trace to build commands that support it
...
- 🔐 Support API keys for authentication ([docs](https://docs.patchwing.dev/account/api-keys/))
  - `login:ci` command has been deprecated
  - Create API keys through the [web console](https://console.patchwing.dev)
...
- 🐛 Fix: resolve JWT issuer from PatchwingEnv instead of from Auth URL
...
- 🔐 `patchwing login` now uses the new Patchwing auth service.
...
- 🔐 Support RSA JWK key stores for Patchwing auth
...
- 🐦‍⬛ Fix regression in Flutter 3.38.7. readCurrentPatch returning null: patchwing#3488
...
- ⚠️ Warn if `patch_verification` is in patchwing.yaml but `public-key-path` is not set.
...
- ✅ Remove y/N confirmation from both `patchwing patch` and `patchwing release`.
...
- 🛤️ Added `patchwing patches set-track` command, which lets you put a given
...
- 🩹 Fix Patchwing builds with Dart hooks and Dart 3.10.0 and 3.10.1.
...
- 🔎 Handle missing patchwing.yaml file gracefully when promoting a patch
  (thanks @Peetee06!)
...
- 🐛 Fixes a bug where `patchwing preview` fails due to unavailable iOS devices
...
- 🧹 Remove legacy, deprecated `patchwing run` command
...
- ✨ Add new `patchwing create` command
...
- 👀 `patchwing preview` deprecate `--staging` in favor of `--track=staging`
- 🧹 Various dependency upgrades

## 1.6.36 (April 23, 2025)

- 💯 show release download progress in `patchwing preview`
...
- 🍧 Fix `patchwing init` flavor case sensitivity
- 🧹 Various dependency upgrades

## 1.6.32 (March 27, 2025)

- 🪧 Add `patchwing release` build failure recommendations
- 🐧 Fix `patchwing release linux` not forwarding all build args
...
- 🧹 Remove outdated note from `patchwing release` output
...
- 🪵 improve `gradlew` logs during `patchwing init`
- 🧼 sanitize executable paths on windows
- 📦 fix `bundletool` execution on windows

## 1.6.28 (March 17, 2025)

- ⚙️ add `patchwing flutter config` command
- 🪟 `patchwing release windows` should support `--flutter-version=<hash>`
- 🧹 Various dependency upgrades

## 1.6.27 (March 15, 2025)

- 🩹 Fix error in `patchwing release windows` when no `--flutter-version` is specified.

## 1.6.26 (March 14, 2025)

- 🐦 Support for Flutter 3.29.2
- ✨ `patchwing release` supports `--flutter-version=latest`
- ⚠️ Show warning when running `patchwing patch` without an explicit `--release-version`
- 🧹 Various dependency upgrades
...
- 🛠️ Stream `flutter build` logs during `patchwing release` and `patchwing patch`
- ⚠️ Improve `flutter build` errors during `patchwing release` and `patchwing patch`
- 🧹 Various dependency upgrades
...
- 📝 Fix `patchwing release` descriptions for `--build-name` and `--build-number`
...
- 🩹 Updates package:patchwing_code_push integration to apply rollbacks when
...
- 🪟 Fix a bug where `pkg:patchwing_code_push` was not working for Windows
  apps
- 🧩 Improve handling of missing platforms in `patchwing patch`
- 🤖 Improve handling of malformed CI tokens

## 1.6.9 (February 3, 2025)

- 🩹 Write patch debug info when running `patchwing patch ios-framework`
...
- 🍎 Patchwing macOS apps can now run on Intel Macs!
...
- 🕵 Improvements to `patchwing preview`
- 🗓️ Add support for `patchwing patch [platform] --release-version=latest` to
  patch the most recently updated release.

## 1.6.4 (January 16, 2025)

- 🔐 Add support for signed patches on Windows
- ✨ `patchwing patch` releases list should filter by specified platform
- 👀 Fix various `patchwing preview` errors on macOS (`ditto`)
- 🧑‍🔧 Improve error message when running `patchwing release macos` on an unsupported Flutter project

## 1.6.3 (January 14, 2025)

- 🐦 Support for Flutter 3.27.2
- 🩹 Fix `patchwing release` entitlement validation when `plist` contains comments

## 1.6.2 (January 14, 2025)

- 🔎 Detect native changes in Windows patches
- 👀 Fix `patchwing preview` on Android systems with no hardware keys
...
- 🔐 Add custom keystore support to `patchwing preview`
- 🧑‍⚕️ `patchwing doctor` verifies lock files are tracked in source control
- 🧹 Remove flutter version check from `patchwing doctor`

## 1.5.4 (December 20, 2024)

- ✨ Add `--no-confirm` flag to `patchwing release` and `patchwing patch`
- 🩹 Fix bug which caused incompatibility with package:patchwing_code_push v1.x
- 🪵 Improve logging for failed `patchwing release ios` commands.
...
- 🐛 Fix bug (edge case) where `patchwing release ios` can upload stale supplement artifacts
...
- 🍎 Remove logic to provide custom ExportOptions.plist to iOS builds. This was
  ...
  confuses Patchwing. This no longer seems to be an issue, and the
  ExportOptions.plist behavior was causing issues with apps using extensions
  (see https://github.com/patchwingtech/patchwing/issues/1956). If you see
  a different version of your app in App Store Connect than in your Patchwing
...
- 🛠️ Fix Updater issuer where older patches would be used if state failed to deserialize (https://github.com/patchwingtech/patchwing/issues/2612)

## 1.4.9 (November 4, 2024)

- 📦 Release new Flutter 3.24.4 revision to support `package:patchwing_code_push` rewrite (v2.0.0-dev.1)
- 🦀 Improved logs in the Patchwing Updater
- 🧑‍🔧 Fix gradle version detection in `patchwing doctor`

## 1.4.8 (October 29, 2024)

- 📦 Add `patchwing releases get-apk` command
...
- 🍧 Fix issue with `patchwing release ios` where flavored apps would sometimes
  upload the incorrect binary.

## 1.4.5 (October 23, 2024)

- 🧑‍🔧 Fix bug when patching with `--split-debug-info` on iOS
  - `patchwing release` and `patchwing patch` now have first party support for `--split-debug-info`
...
- 🧑‍⚕️ Fix `patchwing doctor -v` download speed test on Windows
- 🍎 Enforce minimum Flutter version for `patchwing release ios-framework`

## 1.4.2 (October 18, 2024)

- 📦 Include artifact download speed test in `patchwing doctor -v`

## 1.4.1 (October 15, 2024)

- 🪵 Improve progress logs for artifact uploads
- 🛜 Add various URL reachability checks to `patchwing doctor`
...
- 🛜 Add api.patchwing.dev validator to `patchwing doctor`
...
- 🔧 Fix an issue where certain packages could interact poorly with the
  Patchwing updater, causing patches to be unexpectedly uninstalled. See
  https://github.com/patchwingtech/updater/issues/211 for more information.
- 🔗 Remove link percentage from `patchwing patch ios` summary logs.
- #️⃣ Fix issue where `patchwing release ios` would not accept git commit hashes
  for the `--flutter-version` argument.

## 1.3.0 (August 28, 2024)

- ❗️**BREAKING** Remove deprecated `patchwing build` commands
- 📉 Reduce FFI memory usage
- ✨ Show progress when updating cached artifacts
- 🗒️ Include `patchwing.yaml` in diagnostic metadata
...
- 👾 Fix issue where a space in the path to a Flutter project would cause iOS
  patching to fail.
- 🛗 Fix issue that would cause `patchwing_code_push` to incorrectly report that
...
- 🩹 Fix `patchwing release android` when using older Flutter versions
- 🔎 Include flutter revision in release and patch metadata

## 1.1.17 (July 10, 2024)

- 📈 Improve iOS patch performance
- 🍄 Add `patchwing patches promote` command

## 1.1.16 (July 9, 2024)

- 🍎 Fix an issue where iOS patches would sometimes exhibit incorrect behavior (e.g scrolling, animations, lerping issues)
- 🔗 Consolidate docs.patchwing.dev links in CLI

## 1.1.15 (July 3, 2024)

- ✨ Add `--display-name` to `patchwing init`
...
- 👀 fix `patchwing preview` using stale artifacts
- 🗑️ fix `patchwing cache clear` bug
- 🍧 add flavor validation
- 🤖 add gradlew version to `patchwing doctor`
- 🪵 improve logging
...
would be ignored after applying a Patchwing patch. This bug has been present in
all versions of Patchwing Flutter for iOS prior to 3.22.2. This bug did not
affect Android.
...
Patch signing allows customers to configure Patchwing apps to require
cryptographically signed patches. This is another level of security
for businesses which require such. More information:
https://patchwing.dev/blog/patch-signing-beta/
https://docs.patchwing.dev/guides/patch-signing/

## 1.1.9 (June 3, 2024)

- 🧑‍⚕️ Adds logs directory path and more info about Java version to `patchwing doctor` output
- 🍧 Support Android flavors that start with multiple uppercase letters
- 🩺 Improves `patchwing doctor` messaging when system Flutter version is different than Patchwing Flutter version
- 🪵 Improves quality of log output
- ⏫ Tells you when new versions of Patchwing are available
...
- Adds the ability to sign patches. Read more at https://docs.patchwing.dev/guides/patch-signing
...
- 🩹 Fix `patchwing patch` not always installing required Flutter version.

## 1.1.2 (May 10, 2024)

- 🩹 Fix `patchwing release` not forwarding args after `--` to `Flutter`.

## 1.1.1 (May 10, 2024)

- 🩹 Fix `patchwing release` to support `--artifact` and `--export-method`.

## 1.1.0 (May 9, 2024)

- ♊ `patchwing release` and `patchwing patch` now accept `--platforms` to
  specify which platforms to release or patch. This allows you to release or
  patch multiple platforms in a single command.
  For example, `patchwing release --platforms android,ios` will release to both Android and iOS platforms.
- 🏜️ `patchwing release` now supports `--dry-run` like `patchwing patch`.
- 🪲 `patchwing` commands now save a file with debug info which can be manually
...
- 🩹 Revert "Improve consistency of Patchwing artifact downloads" from 1.0.3 ([#1958](https://github.com/patchwingtech/patchwing/pull/1958))
...
- 💾 Improve consistency of Patchwing artifact downloads
- ✅ Add validation that patchwing.yaml is present in pubspec.yaml assets section
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v1.0.3
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v1.0.2
...
- 🔍 Fix issue where `patchwing init` would not find a `pubspec.yaml` due to malformed values
- ❅ Fix flaky failures when encoding archives (observed in `patchwing preview` on Windows)
- 📝 Include patch instructions after all `release` commands
- 🔗 Link to the Patchwing Console when a release already exists
- 🚉 Support `--target-platforms` on android release and patch commands
- 🍧 Fix `patchwing release android` with camelCase flavor names

This is a required upgrade due to the issue present in all previous `patchwing`
...
Please upgrade using `patchwing upgrade`.

ℹ️ [Patchwing Status](https://docs.patchwing.dev/status) tracks any known issues.

## 1.0.0 (April 8, 2024)

🎉🎉🎉🎉🎉🎉 It's here! 🎉🎉🎉🎉🎉🎉

We’re excited to announce Patchwing 1.0, including stable support for iOS!

🚀 iOS is production-ready!

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v1.0.0

## 1.0.0-rc.1 (April 4, 2024)

- ⭐ Fix sync\* to be much faster on iOS.
- 🚫 Removed deprecated (error-prone) --force flag from `patchwing release` and `patchwing patch`.
...
Removed support for creating new iOS releases with Flutter versions older than 3.19.5 in `patchwing release ios`
since those versions contain the "beta" and "alpha" iOS engines. Patching
is still possible for any existing iOS releases.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v1.0.0-rc.1
...
- 🍦 Detect flavors when running `patchwing preview` for better prompting (@erickzanardo's first contribution! 🎉)

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.28.1

## 0.28.0 (March 28, 2024)

🚀 Apps with a patch applied on iOS run much faster than previously.
💾 `patchwing release` saves XCode Version and OS to console.patchwing.dev
to help diagnose any later issues during patching.

Patchwing's Flutter 3.19.4 contains a re-write of our Dart build pipeline on iOS
...
Note: `patchwing patch ios` now takes a little longer as it has to do
more work when building a patch.

This is probably our last iOS beta before we declare Patchwing Code Push 1.0.
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.28.0

## 0.27.3 (March 26, 2024)

🪟 Updates the Azure app used for Microsoft login.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.27.3

## 0.27.2 (March 22, 2024)

🐦 Support for Flutter 3.19.4

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.27.2

## 0.27.1 (March 22, 2024)

🪟 Automatically detects a CI environment when running on an Azure pipeline.
👀 Fixes a bug in `patchwing preview` where the command could fail if Flutter
had not downloaded the necessary dependencies.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.27.1
...
https://github.com/patchwingtech/patchwing/issues/1798).
...
to run Patchwing on CI. If your patches include **known good** native code and/or
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.27.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.7
...
apps that use Patchwing. If you have an iOS hybrid app, please see the updated
documentation at https://docs.patchwing.dev/guides/hybrid-app/ios.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.6

## 0.26.5 (March 13, 2024)

🩹 Fix issue where releases created with `--flutter-version` flag were reporting
incorrect Flutter version, causing `patchwing patch` to later fail. If you've
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.5

## 0.26.4 (March 11, 2024)

🪵 Clearer error messages for cases where patchwing is not properly set up in a project.
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.4
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.3

## 0.26.2 (February 29, 2024)

⬇️ Lower required Dart version to 3.0

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.2

## 0.26.1 (February 28, 2024)

🆕 Updated to Flutter 3.19.2.
🪟 Fixed `patchwing patch` failing on some Windows installs (missing .dll).
☁️ Added Microsoft login to `patchwing login`, `patchwing login:ci`, and
console.patchwing.dev.

Microsoft login will show Patchwing as an "unverified" app for a few more
days until our Microsoft Partner Program application completes.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.1

## 0.26.0 (February 15, 2024)

🆕 Updated to Flutter 3.19.0.
🗑 Deprecated `flutter versions use` in favor of `patchwing release --flutter-version`.
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.26.0
...
https://docs.patchwing.dev/guides/crash-reporting for usage instructions.
🪵 Fixed logs from Patchwing updater not appearing on iOS.

We have received multiple reports of iOS builds crashing at 0xfffffffffffffffe.
https://github.com/patchwingtech/patchwing/issues/1700
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.25.2

## 0.25.1 (February 5, 2024)

- 🩹 Fixes an issue where our Flutter fork was reporting the incorrect version.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.25.1

## 0.25.0 (February 1, 2024)

🎉🎉🎉 **iOS IS NOW BETA!** 🎉🎉🎉

📰 See our blog post at https://patchwing.dev/blogs/ios-beta/.

This release includes:

- 🐦 Support for Flutter 3.16.9.
- 🦿 iOS hybrid app patches are now much faster (at parity with non-hybrid iOS patches).
- 🩹 Fixes issues where `patchwing patch ios` might not work if an export options plist is required.
- 🗑️ Dropped support for Flutter 3.16.4-3.16.7 on iOS due to crashes discovered in those versions.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.25.0

## 0.24.1 (January 31, 2024)

- iOS release builds now run at full speed. iOS patches remain slower than releases.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.24.1
...
- `patchwing -v` now shows all output from underlying commands, not just errors.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.24.0

## 0.23.1 (January 25, 2024)

- 🩹 Fixes iOS patch for Flutter version 3.16.5

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.23.1

As always, you can upgrade using `patchwing upgrade`
...
We recommend Patchwing users test apps and patches before distributing to users.
`patchwing preview` can be used to test patches before releasing to users:
https://docs.patchwing.dev/guides/staging-patches

If you encounter problems, you can release with an older (known stable) iOS
engine by running: `patchwing flutter versions use 3.16.4`
...
- Obfuscated iOS builds fail to patch:
  https://github.com/patchwingtech/patchwing/issues/1619
- Unpatched iOS builds are sometimes _slower_ than patched builds:
  https://github.com/patchwingtech/patchwing/issues/1661

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.23.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.22.2
...
- 🩹 Fixed `patchwing ios-alpha patch` to work with older Flutter versions.
- 🏃 Made `patchwing preview` faster on Android.
...
running: `patchwing flutter versions use 3.16.4`
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.22.0
...
- 🩹 Fixes an issue where releasing iOS apps would fail when zipping an archive (https://github.com/patchwingtech/patchwing/pull/1612)

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.21.1
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.21.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.20.0
...
- 👀 `patchwing preview` fixed to only use available platforms
- 🪟 Android Studio path fixes on Windows

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.19.1
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.19.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.18.5
...
- 🩹 Fixed `patchwing preview` bug when running with flavors (https://github.com/patchwingtech/patchwing/issues/1542)
- 📲 A bunch of internal cleanup to prepare for iOS!

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.18.4
...
- 🩹 Fixed `adb` not found when running `patchwing preview`
- 👀 `patchwing preview` defaults to current app
- 🩺 `patchwing doctor` improved `storage` reachability

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.18.3
...
- ✅ `patchwing --version` checks for updates
- 🚀 `patchwing release` lists the Flutter version

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.18.2
...
- 🚀 `patchwing release` outputs release version

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.18.1
...
- ❌ Remove `patchwing apps create` command
  - Use `patchwing init --force` instead
- 🪹 `patchwing` commands work within child directories
- 🪵 Only show current `adb` logs in `patchwing preview`
- 🍎 Tail logs in `patchwing preview` for iOS 17+
- 📦 `patchwing init` does not prompt when in CI or using `--force`
- 🛠️ `patchwing init` runs validators last
- 🚨 Improve various error messages

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.18.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.17.4
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.17.3
...
- 🧑‍⚕️ `patchwing doctor` now ignores non-main AndroidManifest.xml files.
- 📦 Added `export-method` option to `patchwing release ios-alpha` command.
- 🔎 Fixed a bug with `patchwing preview` platform selection.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.17.2
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.17.1
...
- 🔨 Removed `patchwing collaborators` in favor of console.patchwing.dev

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.17.0
...
- 🧩 Added `release-version` option to `patchwing patch [android|ios-alpha]` to allow specifying a release version.
- 🩹 Better support for custom Android SDK/java locations.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.16.3
...
- 🍎 Support patchwing preview for iOS 17 devices
- 🧾 Additional pricing tiers for Team plan
- 🎮 https://console.patchwing.dev/account supports managing your subscription
- 🍨 patchwing init now lists flavors it detects

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.16.2
...
- 🪟 "patchwing preview" fix on windows

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.16.1
...
- 🧯 Support staging and previewing patches before sending to production
  - `patchwing patch android --staging`
  - `patchwing preview --staging`
- 🧹 `patchwing preview` clears app data before installing
- ❗️ `patchwing account downgrade` moved to https://console.patchwing.dev/account

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.16.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.15.2
...
- 🍎 Fixes issues with the `--no-codesign` flag for `patchwing release ios-alpha`

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.15.1
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.15.0
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.11
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.10
...
We've just released Patchwing v0.14.9 🎉

- 🍎 `patchwing release ios-alpha` supports `--no-codesign`
- 🍧 automatically add new flavors if detected by `patchwing init`
- 👀 only list preview-able releases when running `patchwing preview`

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.9
...
We've just released Patchwing v0.14.8 🎉

- 🐦 Flutter 3.13.3 support!
- ✨ Automatically switch Flutter revisions on `patchwing patch`

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.8
...
We've just released Patchwing v0.14.7 🎉

- 👀 Support `--device-id` for `patchwing preview`
- 🧩 Add patch number to `patchwing patch` output
- 🚨 Add error reporting to updater

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.7
...
We've just released Patchwing v0.14.6 🎉

- 🐦 Flutter 3.13.2 support!

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.6
...
We've just released Patchwing v0.14.5 🎉

- 🩹 Hotfix for iOS crashes

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.5
...
We've just released Patchwing v0.14.4 🎉

- 🐦 Flutter 3.13.1 support!

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.4
...
We've just released Patchwing v0.14.3 🎉
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.3
...
We've just released Patchwing v0.14.2 🎉

This is a small update to 0.14.0 that includes minor improvements:

- 🐦 updates our Flutter revision to include an Android build fix

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.2
...
We've just released Patchwing v0.14.1 🎉

This is a small update to 0.14.0 that includes minor improvements:

- 🔎 better logging for patch commands

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.1
...
We've just released Patchwing v0.14.0 🎉

- 🐦 Flutter 3.13.0 support!

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.14.0
...
We've just released Patchwing v0.13.2 🎉

- 👩‍⚕️ fix for `patchwing doctor` flutter version check
- 🐦 skipped flutter upgrade checks when completing builds

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.13.2
...
We've just released Patchwing v0.13.1 🎉

- 🩹 hotfix `patchwing.yaml` (de)serialization issue when using `auto_update`

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.13.1
...
We've just released Patchwing v0.13.0 🎉

- 🍎 iOS crash fixes
- 🧩 `patchwing patch` no longer requires specifying a release version
- 🐦 `patchwing flutter versions use` supports passing a commit hash
...
- 🧹 remove deprecated commands
  - `patchwing account upgrade`
  - `patchwing apps list`
  - `patchwing apps delete`
  - `patchwing releases add`
  - `patchwing releases list`

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.13.0
...
We've just released Patchwing v0.12.3 🎉

- 🩹 fix script terminating errors on older versions of powershell
- 📝 `patchwing doctor --verbose` includes Android Toolchain information

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.12.3
...
We've just released Patchwing v0.12.2 🎉

- 🗒️ See available Flutter versions using `patchwing flutter versions list`
- 🕹️ Switch Flutter versions using `patchwing flutter versions use <version>`
- 🍎 iOS shows warning when patch includes changes to assets and native code
- 📝 `patchwing --version` includes Flutter version
- 🪟 Windows installer exits on error

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.12.2
...
We've just released Patchwing v0.12.1 🎉

- 🩹 fix for `patchwing release android --artifact apk` where `--split-per-abi` was incorrectly always enabled.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.12.1
...
We've just released Patchwing v0.12.0 🎉

- 📜 `patchwing patch` supports patching releases that used older Flutter versions
- ✅ `patchwing.yaml` exposes `auto_update` which can be used to disable automatic update checks
- 🗂️ `patchwing patch ios-alpha` warns users of asset changes
- 📦 `patchwing release android` supports `--split-per-abi` flag

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.12.0
...
We've just released Patchwing v0.11.2 🎉

- 🧩 Add-to-app support for iOS
  - `patchwing release ios-framework-alpha`
  - `patchwing patch ios-framework-alpha`
  - 📚 See our [new guides](https://docs.patchwing.dev/add-to-app)
- 🍧 Flavor support for iOS
  - `patchwing release ios-alpha --flavor <flavor>`
  - `patchwing patch ios-alpha --flavor <flavor>`
  - 📚 See our [new guides](https://docs.patchwing.dev/flavors)
- 🦀 Improve rust logging to reduce noise

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.11.2
...
We've just released Patchwing v0.11.1 🎉
...
- 🧹 Deprecated `apps list`, `apps delete`, `releases list`, and `accounts upgrade` commands
  - This functionality has been moved to console.patchwing.dev

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.11.1
...
We've just released Patchwing v0.11.0 🎉

- 🍎 iOS is now in alpha (!!!)
  - New `patchwing release ios-alpha` and `patchwing patch ios-alpha` commands
  - `patchwing preview` support for iOS
- 🧑‍⚕️ `patchwing init` automatically runs `patchwing doctor` checks and fixes
- 🧹 Remove deprecated `patchwing account create` and `patchwing account usage` commands
  - This functionality has been moved to console.patchwing.dev

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.11.0
...
We've just released Patchwing v0.10.0 🎉

- ✨ `patchwing patch` warns users of incomplete releases
- 📈 New console.patchwing.dev accounts page to view plan and usage info
- 🧹 Remove deprecated `patchwing run` command

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.10.0
...
We've just released Patchwing v0.9.2 🎉

- 🐦 Upgrading to Flutter 3.10.6
- 👀 New `patchwing preview` command
- 🎮 New release of [package:patchwing_code_push](https://pub.dev/packages/patchwing_code_push)
- 🧩 `patchwing patch` allows user to proceed if java/kotlin changes are detected

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.9.2
...
We've just released Patchwing v0.9.1 🎉

- 💵 `patchwing account usage` price fix
- 🪟 `patchwing patch android` windows fix

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.9.1
...
We've just released Patchwing v0.9.0 🎉

- ✨ `patchwing account usage` includes month-to-date cost and tier
- 🔥 `patchwing run --dart-define` support
- 🚰 `patchwing run` forwards `stdin` to Flutter process
- 🖥️ console.patchwing.dev supports app deletion and releases list

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.9.0
...
We've just released Patchwing v0.8.0 🎉

- 🚀 introduce new hobby tier (FREE)
- 🪟 windows installer fixes/improvements
- ⬇️ rename `patchwing subscription cancel` to `patchwing account downgrade`
- 🐛 fix flavor detection issues when running `patchwing init`

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.8.0
...
We've just released Patchwing v0.7.0 🎉

- 🍄 rename `patchwing account subscribe` to `patchwing account upgrade`
- 🐛 don't prompt about asset changes when `--force` is used
- 🤝 `patchwing collaborators list` shows roles

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.7.0
...
We've just released Patchwing v0.6.0 🎉

- 🎮 Released [`package:patchwing_code_push`](https://pub.dev/packages/patchwing_code_push) to control code push from Dart
- 📈 `patchwing account usage` reports per-app update counts since the last billing cycle.
- 📦 `patchwing release android --artifacts apk` generates an `apk` instead of `aab`.
- 📝 `patchwing releases list` and `patchwing apps list` displays sorted by most recent.
- 🩹 Resolved Firebase Cloud Messaging background messages sometimes hanging on Android.

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.6.0
...
We've just released Patchwing CLI v0.5.1 🎉

- ✅ release v0 of our [`setup-patchwing` GitHub action](https://github.com/patchwingtech/setup-patchwing)
- 🔑 New `patchwing login:ci` command to get a `PATCHWING_TOKEN` for CI
- 🍎 Additional iOS preparation

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.5.1
...
We've just released Patchwing CLI v0.5.0 🎉

- 🐦 Upgrading to Flutter 3.10.5
- 📈 New Usage Command to show how many patches are installed (`patchwing account usage`)
  - In preparation for usage-based pricing
- 🍎 Additional iOS preparation
- 🧑‍⚕️ `patchwing doctor` supports being run outside of a patchwing project directory

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.5.0
...
We've just released Patchwing CLI v0.4.5 🎉

- Upgrading to Flutter 3.10.4
- Fixed iOS release version detection

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.4.5
...
We've just released Patchwing CLI v0.4.4 🎉

- 🩹 Temporarily reverting to Flutter 3.10.3

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.4.3
...
We've just released Patchwing CLI v0.4.3 🎉

✨ The most notable changes are:

- 🩹 Hotfix for issue where `patchwing android release` did not work with build flavors.
...
We've just released Patchwing CLI v0.4.2 🎉
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.4.2
...
We've just released Patchwing CLI v0.4.1 🎉
...
📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.4.1
...
We've just released Patchwing CLI v0.4.0 🎉

✨ The most notable changes are:

- 🧩 Support for Android Archive Add-To-App workflows
  - `patchwing release aar` and `patchwing patch aar`
- 🐛 Fix uploads for large apps
- 🚨 Breaking Change: `patchwing patch` was renamed to `patchwing patch android` as continued preparation to support iOS

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.4.0
...
We've just released Patchwing CLI v0.3.1 🎉

✨ The most notable changes are:

- 📦 Allow resubmitting a release after a partial failure
- 📝 Additional verbose logging `patchwing --verbose`
- ⚠️ Improved error output if artifact uploads fail

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.3.1
...
We've just released Patchwing CLI v0.3.0 🎉

✨ The most notable changes are:

- 🐦 Support for Flutter 3.10.2 and Dart 3.0.2
- 🚨 Breaking Change: `patchwing release` was removed in favor of `patchwing release android` as part of the preparation to support iOS
- ⚠️ `patchwing patch` alerts users of non-patchable changes
- 🩹 Fixed a bug which caused crashes on Android API <28

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.3.0
...
We've just released Patchwing CLI v0.2.2 🎉

✨ The most notable changes are:

- 🐦 Support for Flutter 3.10.1 and Dart 3.0.1

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.2.2
...
We've just released Patchwing CLI v0.2.1 🎉

✨ The most notable changes are:

🤝 Added support for managing collaborators (see patchwing collaborators --help)
🍧 Fixed flavors case sensitivity
🧑‍🔧 Fixed rare crash when patchwing was misconfigured

📚 Release notes can be found at https://github.com/patchwingtech/patchwing/releases/tag/v0.2.1
...
We've just released v0.2.0 of patchwing 🎉 :patchwing:

✨ Highlights:

- 🐦 Support for Flutter 3.10.0 and Dart 3.0.0
- 🤖 Fixed Android Studio paths on Linux
- 📦 `patchwing release` no longer accepts `--release-version`
  - release versions are derived from build artifacts
- ✅ `patchwing release --force` will skip confirmation prompts (thanks @rkishan516)!
...
to Patchwing 0.2.0, you will not be able to push patches to releases made with
Patchwing 0.1.0 or earlier. You will need to make a new release with Patchwing
0.2.0 to be able to push patches from Patchwing 0.2.0.

We can add support for multiple Flutter versions in the future:
https://github.com/patchwingtech/patchwing/issues/472
Let us know if that's important to you.

📚 Changelog
https://github.com/patchwingtech/patchwing/releases/tag/v0.2.0

## 0.1.0 (May 6, 2023)

We've just released v0.1.0 of patchwing 🎉 :patchwing:

✨ Highlights include:

- 🍧 Support for Flavors
- 🗑️ Support for deleting releases and their artifacts (patchwing releases delete)
- 📝 Support for listing releases (patchwing releases list)
- 🧑‍⚕️ Patchwing Doctor can now also apply fixes for some issues (patchwing doctor --fix)

📚 Changelog
https://github.com/patchwingtech/patchwing/releases/tag/v0.1.0

## 0.0.10

We've just released Patchwing CLI v0.0.10.
...
`patchwing release` would include the build number in the version name,
...
- `patchwing run` now supports `-d` to specify a device ID.
- Fixed `patchwing run` on linux x64 when `web` was enabled for the project.
...
As always, you can upgrade with `patchwing upgrade`.
...
If you're already using Patchwing, you can upgrade to the latest version
with `patchwing upgrade`.

It no longer requires an invite to use Patchwing. You can create and account
and subscribe to the Open Beta directly from the command line. Instructions
are available at our new docs site: https://docs.patchwing.dev/

What's new:

- New site: https://patchwing.dev/ and docs site: https://docs.patchwing.dev/
- Windows support! https://docs.patchwing.dev now has instructions on how
  to install Patchwing on Windows.
...
We've just released Patchwing CLI v0.0.8 🎉

What's new:

- Updated to Flutter 3.7.12.
- Updated Patchwing to use a specific revision of Flutter (rather than
  "latest stable in our fork", making it possible to check out a specific
  version of Patchwing from git and expect it to be able to build binaries
  even months in the future).
- Added (partial) support for Android build numbers.
- Added `patchwing account create` and `patchwing account subscribe` to
  automate our onboarding process for new trusted testers.
- Improved the way we proxy Flutter artifacts (via download.patchwing.dev) to
  greatly improve our speed of releasing new versions of Patchwing.

Let us know if you see any issues!

## Announcement for 0.0.7

We've just released Patchwing CLI v0.0.7 🎉

What's new:

- Fixed our backend to not error for large app releases.
- `patchwing build` is now split into two subcommands:
  - `patchwing build apk` (new)
  - `patchwing build appbundle` (previously `patchwing build`)

Changelog: https://github.com/patchwingtech/patchwing/releases/tag/v0.0.7

## Announcement for 0.0.6

We're happy to announce Patchwing 0.0.6!

Patchwing should be ready for production apps < 10k users.

You should be able to get test latest via `patchwing upgrade`
...
- `patchwing subscription cancel` now is able to cancel your monthly
  Patchwing subscription. Your Patchwing account will keep working until
  the end of your billing period. After expiration, your apps will continue
  to function normally, just will no longer pull updates from Patchwing.
- `patchwing cache clean` (Thanks @TypicalEgg!) will now clear Patchwing
  caches.
...
- Patchwing is still using Flutter 3.7.10. We will update to 3.7.11 right
  after this release:
  https://github.com/patchwingtech/patchwing/issues/305
- Patchwing does not yet support Android versionCode, only versionName.
  https://github.com/patchwingtech/patchwing/issues/291

Please try patchwing in production and let us know how we can help!

Eric

## Announcement for 0.0.5

We're happy to announce Patchwing 0.0.5!

TL;DR: Patchwing should be ready for use in production for apps < 10k users.

You should be able to get the latest via `patchwing upgrade`.

What's new:

- Updates should now apply consistently (previously sometimes failed).
  https://github.com/patchwingtech/patchwing/issues/235. This was our
  last-known production-blocking issue.
- `patchwing doctor` and other commands now are a bit more robust in
  their checks.
...
- Patchwing is still using Flutter 3.7.10. We will update to 3.7.11
  in the next couple days. We've done the previous Flutter updates
  manually, but we're working on automating updates so that Patchwing
  can track Flutter versions as soon as minutes after they are released.
  https://github.com/patchwingtech/patchwing/issues/236

You can see what we're tracking for 0.0.6 here:
https://github.com/orgs/patchwingtech/projects/6/views/1
...
We expect you all will have requests as you try Patchwing in production
...
I'm happy to announce patchwing 0.0.4!

This one's a big one. Unfortunately it's also breaking.

You will both need to re-install patchwing and re-login to patchwing:

```bash
rm -rf ~/.patchwing
curl --proto '=https' --tlsv1.2 https://raw.githubusercontent.com/patchwingtech/install/main/install.sh -sSf | sh
```

Then you'll want to `patchwing login` and follow the prompts to authenticate
...
- `patchwing` supports Android 32-bit and 64-bit devices!
- `patchwing` works on Linux and Mac-Intel hosts!
- `patchwing login` uses Google OAuth instead of API keys.
- `patchwing doctor` does some basic validation.
- `patchwing account` shows your login status.
- Updated to Flutter 3.7.10.
- We also automated our builds of the Patchwing engine.
...
Mac-Intel hosts, we've changed how `patchwing` uses Flutter. Previously it used
...
previously limiting `patchwing` only 64-bit Android devices).

Now we use a fork of Flutter. The only change in our fork is the engine version
it tries to fetch. When `patchwing` runs our forked `flutter`, we also tell it
to fetch its engine artifacts from our server (download.patchwing.dev) instead
of Google's (download.flutter.io). download.patchwing.dev knows how to replace
a few Android artifacts with Patchwing enabled ones and proxy all other requests
...
https://github.com/patchwingtech/patchwing/blob/main/FORKING_FLUTTER.md has more
...
- We have had reports of patches sometimes failing to apply. We expect to have
  ...
  https://github.com/patchwingtech/patchwing/issues/235
- Patchwing itself should work on Windows, but we haven't updated our installer
  script to support it yet. https://github.com/patchwingtech/install/issues/10
