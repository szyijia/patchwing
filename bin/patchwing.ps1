...
# compiles `patchwing_cli/bin/patchwing_cli.dart` to `bin/cache/patchwing.snapshot`

$ErrorActionPreference = "Stop"

# We are running from $patchwingRootDir\bin
$patchwingBinDir = (Get-Item $PSScriptRoot).FullName
$patchwingRootDir = (Get-Item $patchwingBinDir\..\).FullName
$flutterVersion = Get-Content "$patchwingBinDir\internal\flutter.version"
$patchwingCacheDir = [IO.Path]::Combine($patchwingRootDir, "bin", "cache")
$patchwingCliDir = [IO.Path]::Combine($patchwingRootDir, "packages", "patchwing_cli")
$snapshotPath = [IO.Path]::Combine($patchwingCacheDir, "patchwing.snapshot")
$stampPath = [IO.Path]::Combine($patchwingCacheDir, "patchwing.stamp")
$flutterPath = [IO.Path]::Combine($patchwingCacheDir, "flutter", $flutterVersion)
$flutter = [IO.Path]::Combine($patchwingCacheDir, "flutter", $flutterVersion, "bin", "flutter.bat")
$patchwingScript = [IO.Path]::Combine($patchwingCliDir, "bin", "patchwing.dart")
...
function Test-PatchwingNeedsUpdate {
    ...
    $pubspecFile = [System.IO.FileInfo] "$patchwingCliDir\pubspec.yaml"
    $pubspecLockFile = [System.IO.FileInfo] "$patchwingRootDir\pubspec.lock"

    Push-Location $patchwingRootDir
...
function Update-Flutter {
    ...
    if (!(Test-Path $flutterPath)) {
        Invoke-SilentlyIfNeeded {
            git clone --filter=tree:0 https://github.com/patchwingtech/flutter.git --no-checkout "$flutterPath" 
        }
    }
    else {
        Invoke-SilentlyIfNeeded {
    ...
    # 走 Patchwing 自家 CDN，触发一次 flutter 命令以下载 Dart SDK 等依赖。
    # 允许通过 PATCHWING_STORAGE_URL / FLUTTER_STORAGE_BASE_URL 环境变量覆写，
    # 缺省回退到 https://cdn.patchwing.net。
    $resolvedStorageUrl = if ($env:FLUTTER_STORAGE_BASE_URL) {
        $env:FLUTTER_STORAGE_BASE_URL
    } elseif ($env:PATCHWING_STORAGE_URL) {
        $env:PATCHWING_STORAGE_URL
    } else {
        'https://cdn.patchwing.net'
    }
    $previousStorageUrl = $env:FLUTTER_STORAGE_BASE_URL
    $env:FLUTTER_STORAGE_BASE_URL = $resolvedStorageUrl
    & $flutter --version
    if ($previousStorageUrl) {
        $env:FLUTTER_STORAGE_BASE_URL = $previousStorageUrl
    } else {
        Remove-Item Env:\FLUTTER_STORAGE_BASE_URL
    }
}

function Update-Patchwing {
    Push-Location $patchwingRootDir
    $compileKey = & { git rev-parse HEAD } -split
    Pop-Location
    ...
    (Get-Item "$patchwingRootDir/pubspec.lock").LastWriteTime = Get-Date
    ...
    # assumptions we make about the cwd in the patchwing_cli tool.
    & $dart --verbosity=error --disable-dart-dev --snapshot="$snapshotPath" `
        --snapshot-kind="app-jit" --packages="$patchwingRootDir/.dart_tool/package_config.json" `
        --no-enable-mirrors "$patchwingScript" completion > $null
...
if (Test-PatchwingNeedsUpdate) {
    Update-Patchwing
}

& $dart --disable-dart-dev --packages="$patchwingRootDir\.dart_tool\package_config.json" "$snapshotPath" $args
